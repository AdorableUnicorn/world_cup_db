...
echo "$($PSQL "SELECT(SELECT SUM(winner_goals) FROM games) + (SELECT SUM(opponent_goals) FROM games) as grand_total;")"
...
echo "$($PSQL "SELECT name AS all_teams FROM ( SELECT teams.name FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id WHERE games.round='Eighth-Final' AND games.year=2014 UNION SELECT teams.name FROM teams RIGHT JOIN games ON teams.team_id = games.opponent_id WHERE games.round='Eighth-Final' AND games.year=2014) AS combined_teams ORDER BY name;")"
